...
re-emits it as `kasual-vpad`, and nothing else in the suite proves that the re-emitted
pad actually reaches a foreign application in the foreground: `kcd` and `w3` hand the
screen to a game and stay out of the way. Here every press has to land in Steam, and
Steam is asked after each one where its focus went — the same read-back discipline
`navigation.py` applies to KD, over the debugger Steam's Chromium exposes.
...
def _body(session: Session) -> None:
    ...
    steam_ui.focus_game(steam, session.pad, GAME)
