...
from infrastructure.kde.qt.desktop.app_windows import has_mapped_window, app_window_fullscreen
...
class DeferredHide(QObject, LaunchHide, metaclass=ProtocolQtMeta):
    ...
    A fullscreen window covers a layer-shell TOP surface (KWin stacks fullscreen
    xdg-toplevels above it), so ceding — staying mapped on TOP with
    ...
    def _on_windows(self, windows: list[Window]) -> None:
        ...
        logger.error("DBG DeferredHide._on_windows: app=%s fullscreen=%s windows=%s",
                     app.id, self._fullscreen,
                     [(w.id, w.resource_class, w.fullscreen, w.covers_screen, w.pid) for w in windows])
        self._stop_watch()
        if self._grace_ms > 0:
    ...
    def _act_now(self) -> None:
        self._app = None
        action = "cede" if self._fullscreen else "hide"
        logger.error("DBG DeferredHide._act_now: -> %s", action)
